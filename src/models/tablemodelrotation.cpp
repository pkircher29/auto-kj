...
QVariant TableModelRotation::getBackgroundRole(const QModelIndex &index) const {
    ...
    if (singer.id == m_currentSingerId) {
        if (index.column() > 0)
...
QVariant TableModelRotation::getTooltipData(const QModelIndex &index) const {
    ...
    // Check sung_this_round
    QSqlQuery sungQ;
    sungQ.prepare("SELECT sung_this_round FROM rotationsingers WHERE singerid = :id");
    sungQ.bindValue(":id", singer.id);
    bool sungThisRound = (sungQ.exec() && sungQ.next() && sungQ.value(0).toInt() != 0);
    ...
    if (m_currentSingerId == singer.id) {
        ...
        if (sungThisRound)
            toolTipText += " - Already sang this round";
...
int TableModelRotation::singerAdd(const QString &name, const int positionHint) {
    ...
    const auto &curSingerRef = getSinger(m_currentSingerId);
    int curSingerPos = curSingerRef.isValid() ? curSingerRef.position : -1;
    switch (positionHint) {
        case ADD_FAIR: {
            int fairPos = findFairPosition(singerId);
            if (fairPos != -1 && fairPos != addPos) {
                singerMove(addPos, fairPos);
                singerMoved = true;
            }
            break;
        }
        case ADD_NEXT:
            if (curSingerPos >= 0 && curSingerPos != static_cast<int>(m_singers.size()) - 2) {
...
int TableModelRotation::positionWaitTime(int position) const {
    if (position < 0 || m_singers.empty() || position > static_cast<int>(m_singers.size()) - 1)
        return 0;
...
int TableModelRotation::findFairPosition(const int singerId) const {
    const auto &newSinger = getSinger(singerId);
    if (!newSinger.isValid()) return -1;

    int newSung = newSinger.numSongsSung();
    QDateTime newTs = newSinger.addTs;

    const auto &curSinger = getSinger(m_currentSingerId);
    int curPos = curSinger.isValid() ? curSinger.position : -1;

    int n = static_cast<int>(m_singers.size());
    if (n <= 1) return 0;

    // Sequence: (curPos + 1) % n, (curPos + 2) % n, ..., curPos
    for (int i = 1; i < n; ++i) {
        int idx = (curPos + i) % n;
        const auto &s = m_singers.at(idx);
        if (s.id == singerId) continue;

        int sSung = s.numSongsSung();
        if (sSung > newSung) return idx;
        if (sSung == newSung && s.addTs > newTs) return idx;
    }
