...
void TableModelKaraokeSongs::rebuildPlayedTonightCache() {
    m_playedTonightKeys.clear();
    QSqlQuery q;
    q.exec("SELECT DISTINCT spt.canonical_song_key, d.artist, d.title "
           "FROM songs_played_tonight spt "
           "LEFT JOIN dbsongs d ON d.songid = spt.song_id");
    while (q.next()) {
        QString key = q.value(0).toString().trimmed();
        if (key.isEmpty())
            key = okj::canonicalSongKey(q.value(1).toString(), q.value(2).toString());
        if (!key.isEmpty())
            m_playedTonightKeys.insert(key);
    }
}

void TableModelKaraokeSongs::refreshFairnessState() {
    rebuildPlayedTonightCache();
    ...
    switch (role) {
        ...
        case Qt::ForegroundRole: {
            if (m_settings.fairnessEnabled() && m_settings.fairnessEnforceSongNight()) {
                if (isSongBlockedTonight(m_filteredSongs.at(index.row())))
        ...
        case Qt::ToolTipRole: {
            if (m_settings.fairnessEnabled() && m_settings.fairnessEnforceSongNight()) {
                if (isSongBlockedTonight(m_filteredSongs.at(index.row())))
                    return QString("Already sung tonight by another singer");
            }
            return {};
        }
        default:
...
void TableModelKaraokeSongs::loadData() {
    ...
    while (query.next()) {
        auto song = m_allSongs.emplace_back(std::make_shared<okj::KaraokeSong>(okj::KaraokeSong{
                ...
                okj::canonicalSongKey(query.value(1).toString(), query.value(2).toString()),
                query.value(4).toInt(),
...
Qt::ItemFlags TableModelKaraokeSongs::flags(const QModelIndex &index) const {
    ...
    if (m_settings.fairnessEnabled() && m_settings.fairnessEnforceSongNight()) {
        if (isSongBlockedTonight(m_filteredSongs.at(index.row())))
            return Qt::ItemIsEnabled; // greyed out, not selectable or draggable
    }
    return Qt::ItemIsEnabled | Qt::ItemIsSelectable | Qt::ItemIsDragEnabled;
}

bool TableModelKaraokeSongs::isSongBlockedTonight(const std::shared_ptr<okj::KaraokeSong> &song) const {
    const QString key = song->canonicalSongKey.isEmpty()
                            ? okj::canonicalSongKey(song->artist, song->title)
                            : song->canonicalSongKey;
    return m_playedTonightKeys.contains(key);
}
