...
private:
    ...
    Settings m_settings;
    QSet<QString> m_playedTonightKeys;
    QFont m_itemFont;
    int m_itemHeight{20};
    QFont m_headerFont;
    ...
    [[nodiscard]] bool isSongBlockedTonight(const std::shared_ptr<okj::KaraokeSong> &song) const;
    // Internal: rebuild the in-memory played-tonight set from the DB
    void rebuildPlayedTonightCache();

public:
    // Call after a song is played or nightly tracking resets so played-tonight highlights refresh
    void refreshFairnessState();
