...
void MainWindow::setupShortcuts() {
    ...
    connect(&m_scutKPlayNextUnsung, &QShortcut::activated, [&]() {
        ...
        // Notify AutoKJ server and fairness engine of the song now being played
        m_fairnessEngine.onSongPlayed(nextSinger.id, nextSinger.nextSongCoSingers());
        {
            int autoSongId = m_karaokeSongsModel.getIdForPath(nextSongPath);
            m_fairnessEngine.recordSongPlayed(autoSongId, nextSinger.id, nextSinger.name);
            for (const QString &coName : nextSinger.nextSongCoSingers()) {
                const auto &coSinger = m_rotModel.getSingerByName(coName);
                m_fairnessEngine.recordSongPlayed(autoSongId, coSinger.isValid() ? coSinger.id : -1, coName);
            }
        }
    ...
    m_scutPreview.setKey(QKeySequence("Ctrl+P"));
    m_scutPreview.setContext(Qt::ApplicationShortcut);
    connect(&m_scutPreview, &QShortcut::activated, [this]() {
        auto indexes = ui->tableViewDB->selectionModel()->selectedIndexes();
        if (!indexes.isEmpty()) {
            auto song = qvariant_cast<std::shared_ptr<okj::KaraokeSong>>(indexes.at(0).data(Qt::UserRole));
            if (song) previewSong(song->path);
        }
    });
    ...
    m_scutDeletePlSong.setKey(QKeySequence::Delete);
    m_scutDeletePlSong.setContext(Qt::WidgetShortcut);
...
void MainWindow::setupConnections() {
    ...
    connect(&m_songbookApi, &AutoKJServerClient::alertReceived, this, &MainWindow::showAlert);
    ...
    connect(ui->btnBmStreamingServices, &QPushButton::clicked, this, [this]() {
        if (!dlgStreamingServices)
            dlgStreamingServices = new DlgStreamingServices(this);
        dlgStreamingServices->show();
        dlgStreamingServices->raise();
        dlgStreamingServices->activateWindow();
    });
    ...
    auto actionQRCode = new QAction(QIcon(":/theme/Icons/autokjbreeze-dark/actions/22/view-preview.svg"), "Show Venue QR", this);
    connect(actionQRCode, &QAction::triggered, this, &MainWindow::showQRCodeDialog);
    ui->menuTools->addAction(actionQRCode);
    auto tb = addToolBar("Venue QR");
    tb->addAction(actionQRCode);

    // Venue selector toolbar (top-middle)
    m_venueTb = addToolBar("Venue");
    m_venueTb->setMovable(false);
    m_venueTb->addWidget(new QLabel("  Venue: ", this));
    m_comboBoxVenue = new QComboBox(this);
    m_comboBoxVenue->setMinimumWidth(200);
    m_comboBoxVenue->setToolTip("Select active venue");
    m_venueTb->addWidget(m_comboBoxVenue);
    auto *refreshVenueBtn = new QToolButton(this);
    refreshVenueBtn->setIcon(QIcon(":/theme/Icons/autokjbreeze-dark/actions/22/view-refresh.svg"));
    refreshVenueBtn->setToolTip("Refresh venue list");
    m_venueTb->addWidget(refreshVenueBtn);
    auto *manageVenueBtn = new QToolButton(this);
    manageVenueBtn->setIcon(QIcon(":/theme/Icons/autokjbreeze-dark/actions/22/configure.svg"));
    manageVenueBtn->setToolTip("Manage Venues & Shows");
    m_venueTb->addWidget(manageVenueBtn);
    ...
    m_venueTb->addSeparator();
    m_venueTb->addWidget(new QLabel("  DJ: ", this));
    m_comboBoxDj = new QComboBox(this);
    m_comboBoxDj->setMinimumWidth(150);
    m_comboBoxDj->setToolTip("Select currently active DJ profile");
    m_venueTb->addWidget(m_comboBoxDj);
    
    auto *manageDjBtn = new QToolButton(this);
    manageDjBtn->setIcon(QIcon(":/theme/Icons/autokjbreeze-dark/actions/22/im-user.svg"));
    manageDjBtn->setToolTip("Manage DJ Profiles");
    m_venueTb->addWidget(manageDjBtn);
    
    connect(manageDjBtn, &QToolButton::clicked, this, &MainWindow::showManageDjsDialog);
    connect(m_comboBoxDj, QOverload<int>::of(&QComboBox::activated), this, &MainWindow::onDjSelected);
    connect(&m_settings, &Settings::djListChanged, this, &MainWindow::updateDjList);
    
    updateDjList(); // Initial population

    m_venueTb->addSeparator();
    m_checkBoxAcceptingRequests = new QCheckBox("Accept Requests", this);
    m_venueTb->addWidget(m_checkBoxAcceptingRequests);
    m_checkBoxAcceptingRequests->setChecked(m_songbookApi.getAccepting());
    connect(m_checkBoxAcceptingRequests, &QCheckBox::toggled, this, [this](bool checked) {
        m_songbookApi.setAccepting(checked);
    });

    m_venueTb->addSeparator();
    m_btnEndShow = new QToolButton(this);
    m_btnEndShow->setText("End Show");
    m_btnEndShow->setToolTip("End the current active show for this venue");
    connect(m_btnEndShow, &QToolButton::clicked, this, [this]() {
        if (!m_comboBoxVenue || m_comboBoxVenue->currentIndex() < 0)
            return;
        const bool hasActiveShow = m_comboBoxVenue->itemData(m_comboBoxVenue->currentIndex(), Qt::UserRole + 2).toBool();
        if (!hasActiveShow) {
            QMessageBox::information(this, "End Show", "There is no active show for this venue.");
            updateEndShowButtonState();
            return;
        }
        ...
        if (QMessageBox::question(
                this,
                "End Show",
                "End the active show for this venue now?",
                QMessageBox::Yes | QMessageBox::No
            ) != QMessageBox::Yes) {
            return;
        }

        if (m_checkBoxAcceptingRequests && m_checkBoxAcceptingRequests->isChecked()) {
            m_songbookApi.setAccepting(false, false);
            m_checkBoxAcceptingRequests->blockSignals(true);
            m_checkBoxAcceptingRequests->setChecked(false);
            m_checkBoxAcceptingRequests->blockSignals(false);
        }
    ...
    auto actionCheckins = new QAction("Singer Check-ins...", this);
    connect(actionCheckins, &QAction::triggered, this, [this]() {
        auto *dlg = new DlgCheckins(m_rotModel, m_songbookApi, this);
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        dlg->show();
    });
    ui->menuTools->addAction(actionCheckins);

    auto actionAliasManager = new QAction("Singer Alias Manager...", this);
    connect(actionAliasManager, &QAction::triggered, this, [this]() {
        auto *dlg = new DlgAliasManager(this);
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        dlg->show();
    });
    ui->menuTools->addAction(actionAliasManager);

    auto actionDjManager = new QAction("Manage DJ Profiles...", this);
    connect(actionDjManager, &QAction::triggered, this, &MainWindow::showManageDjsDialog);
    ui->menuTools->addAction(actionDjManager);

    auto actionVenueManager = new QAction("Manage Venues & Gigs...", this);
    connect(actionVenueManager, &QAction::triggered, this, &MainWindow::showManageVenuesGigsDialog);
    ui->menuTools->addAction(actionVenueManager);
}

void MainWindow::onVenuesChanged(const OkjsVenues &venues) {
    if (!m_comboBoxVenue)
        return;
    int currentVenueId = m_settings.requestServerVenue();
    m_comboBoxVenue->blockSignals(true);
    m_comboBoxVenue->clear();
    int selectIdx = 0;
    int currentIdx = -1;
    int firstNotAcceptingIdx = -1;
    for (int i = 0; i < venues.size(); ++i) {
        m_comboBoxVenue->addItem(venues.at(i).name, venues.at(i).venueId);
        m_comboBoxVenue->setItemData(i, venues.at(i).urlName, Qt::UserRole + 1);
        m_comboBoxVenue->setItemData(i, venues.at(i).hasActiveShow, Qt::UserRole + 2);
        if (venues.at(i).venueId == currentVenueId)
            currentIdx = i;
        if (!venues.at(i).accepting && firstNotAcceptingIdx < 0)
            firstNotAcceptingIdx = i;
    }
    // Keep the user's selected venue whenever possible.
    // Only auto-pick a different one when the saved venue is no longer present.
    if (currentIdx >= 0) {
        selectIdx = currentIdx;
    } else if (firstNotAcceptingIdx >= 0) {
        selectIdx = firstNotAcceptingIdx;
    }
    m_comboBoxVenue->setCurrentIndex(selectIdx);
    m_comboBoxVenue->blockSignals(false);
    if (!venues.isEmpty()) {
        int savedId = m_comboBoxVenue->itemData(selectIdx).toInt();
        m_settings.setRequestServerVenue(savedId);
        m_settings.setRequestServerVenueSlug(m_comboBoxVenue->itemData(selectIdx, Qt::UserRole + 1).toString());
        if (m_checkBoxAcceptingRequests) {
            m_checkBoxAcceptingRequests->blockSignals(true);
            m_checkBoxAcceptingRequests->setChecked(m_songbookApi.getAccepting());
            m_checkBoxAcceptingRequests->blockSignals(false);
        ...
        updateEndShowButtonState();
    } else if (m_songbookApi.isConnected()) {
        m_logger->info("{} No venues found on connection, forcing setup", m_loggingPrefix);
        ...
        while (!success) {
            ...
            dlg.setWindowTitle("Welcome to AutoKJ - Set up your first venue");
...
void MainWindow::onVenueSelected(int index) {
    if (!m_comboBoxVenue)
        return;
    if (index < 0 || index >= m_comboBoxVenue->count())
        return;
...
void MainWindow::updateEndShowButtonState()
{
    if (!m_btnEndShow) {
        return;
    }
    ...
    if (m_comboBoxVenue && m_comboBoxVenue->currentIndex() >= 0) {
        hasActiveShow = m_comboBoxVenue->itemData(m_comboBoxVenue->currentIndex(), Qt::UserRole + 2).toBool();
    }

    m_btnEndShow->setEnabled(hasActiveShow);
    if (hasActiveShow) {
        m_btnEndShow->setStyleSheet(
            "QToolButton { background-color: #b91c1c; color: white; border: 1px solid #7f1d1d; padding: 4px 10px; }"
            "QToolButton:hover { background-color: #dc2626; }"
            "QToolButton:disabled { background-color: #4b5563; color: #d1d5db; }"
        );
    } else {
        m_btnEndShow->setStyleSheet(
            "QToolButton { background-color: #4b5563; color: #d1d5db; border: 1px solid #374151; padding: 4px 10px; }"
            "QToolButton:disabled { background-color: #4b5563; color: #d1d5db; }"
        );
    }
}

void MainWindow::showQRCodeDialog() {
    if (!m_settings.requestServerEnabled()) {
        QMessageBox::information(this, "QR Code", "Please enable and configure the request server in Settings first.");
        return;
    }
    DlgQRCode dlg(m_songbookApi.venueUrl(), this);
    dlg.exec();
}
...
void MainWindow::dbInit(const QDir &okjDataDir) {
    ...
    QString dbFilePath = okjDataDir.absolutePath() + QDir::separator() + "auto-kj.sqlite";
    m_database.setDatabaseName(dbFilePath);
    m_database.open();
    // Create backup manager and run startup backup
    m_autoBackupManager = new AutoBackupManager(dbFilePath, this);
    m_autoBackupManager->runBackup();
    QSqlQuery query(
    ...
    if (schemaVersion < 107) {
        m_logger->info("{} Updating database schema to version 107 (Co-singer support)", m_loggingPrefix);
        query.exec("ALTER TABLE queueSongs ADD COLUMN cosinger2 TEXT DEFAULT NULL");
        query.exec("ALTER TABLE queueSongs ADD COLUMN cosinger3 TEXT DEFAULT NULL");
        query.exec("ALTER TABLE queueSongs ADD COLUMN cosinger4 TEXT DEFAULT NULL");
        query.exec("PRAGMA user_version = 107");
        m_logger->info("{} DB Schema update to v107 completed", m_loggingPrefix);
    }
    if (schemaVersion < 108) {
        m_logger->info("{} Updating database schema to version 108 (Alias Support)", m_loggingPrefix);
        query.exec("CREATE TABLE IF NOT EXISTS singerAliases ( aliasid INTEGER PRIMARY KEY AUTOINCREMENT, canonical_name TEXT NOT NULL COLLATE NOCASE, alias_name TEXT NOT NULL COLLATE NOCASE UNIQUE, first_seen TIMESTAMP, last_seen TIMESTAMP)");
        query.exec("PRAGMA user_version = 108");
        m_logger->info("{} DB Schema update to v108 completed", m_loggingPrefix);
    }
    if (schemaVersion < 109) {
        m_logger->info("{} Updating database schema to version 109 (Rotation Fairness)", m_loggingPrefix);
        query.exec("ALTER TABLE rotationsingers ADD COLUMN sung_this_round INTEGER DEFAULT 0");
        query.exec("CREATE TABLE IF NOT EXISTS rotation_meta (key TEXT PRIMARY KEY, value TEXT)");
        query.exec("INSERT OR IGNORE INTO rotation_meta (key, value) VALUES ('current_round', '1')");
        query.exec("PRAGMA user_version = 109");
        m_logger->info("{} DB Schema update to v109 completed", m_loggingPrefix);
    }
    if (schemaVersion < 110) {
        m_logger->info("{} Updating database schema to version 110 (Once-per-night song tracking)", m_loggingPrefix);
        query.exec("CREATE TABLE IF NOT EXISTS songs_played_tonight ("
    ...
    if (schemaVersion < 111) {
        m_logger->info("{} Updating database schema to version 111 (Canonical song-night keys)", m_loggingPrefix);
        query.exec("ALTER TABLE songs_played_tonight ADD COLUMN canonical_song_key TEXT");
        query.exec(
            "UPDATE songs_played_tonight "
            "SET canonical_song_key = lower(trim(replace("
...
void MainWindow::play(const QString &karaokeFilePath, const bool &k2k) {
    // Stop any audio preview that may be playing
    if (m_mediaBackendPreview.state() == MediaBackend::PlayingState)
        m_mediaBackendPreview.stop(true);
    m_mediaTempDir = std::make_unique<QTemporaryDir>();
...
void MainWindow::tableViewQueueDoubleClicked(const QModelIndex &index) {
    ...
    // Fairness: warn DJ if singer already sang this round
    {
        QString roundWarning = m_fairnessEngine.checkRoundViolation(singer.id);
        if (!roundWarning.isEmpty()) {
            QMessageBox msgBox(this);
            msgBox.setWindowTitle("Fairness Warning");
            msgBox.setIcon(QMessageBox::Warning);
            msgBox.setText(roundWarning);
            msgBox.setInformativeText("Override and play anyway?");
            QPushButton *overrideBtn = msgBox.addButton("Override", QMessageBox::AcceptRole);
            msgBox.addButton(QMessageBox::Cancel);
            msgBox.exec();
    ...
    // Notify AutoKJ server and fairness engine of the song now being played
    QStringList cosingers;
    if (!song.cosinger2.isEmpty()) cosingers.append(song.cosinger2);
    if (!song.cosinger3.isEmpty()) cosingers.append(song.cosinger3);
    if (!song.cosinger4.isEmpty()) cosingers.append(song.cosinger4);
    m_fairnessEngine.onSongPlayed(singer.id, cosingers);
    m_fairnessEngine.recordSongPlayed(song.dbSongId, singer.id, singer.name);
    for (const QString &coName : cosingers) {
        const auto &coSinger = m_rotModel.getSingerByName(coName);
        m_fairnessEngine.recordSongPlayed(song.dbSongId, coSinger.isValid() ? coSinger.id : -1, coName);
    }
    m_karaokeSongsModel.refreshFairnessState();
...
void MainWindow::actionSettingsTriggered() {
    ...
    connect(settingsDialog, &DlgSettings::requestServerEnableChanged, m_venueTb, &QToolBar::setVisible);
    ...
    connect(settingsDialog, &DlgSettings::requestServerIntervalChanged, &m_songbookApi, &AutoKJServerClient::setInterval);
    connect(settingsDialog, &DlgSettings::maxSingersPerRequestChanged, &m_songbookApi, &AutoKJServerClient::pushGigSettings);
...
void MainWindow::clearRotation() {
    if (m_testMode) {
        ...
        m_rotDelegate.setCurrentSinger(-1);
        m_qModel.loadSinger(-1);
        m_fairnessEngine.resetNightlyTracking();
        m_karaokeSongsModel.refreshFairnessState();
        return;
    }
    QMessageBox msgBox;
...
void MainWindow::rotationDataChanged() {
    ...
    // Push rotation state to AutoKJ server for PWA display
    if (m_songbookApi.isConnected())
    {
        QJsonArray singersArray;
        const int curId = m_rotModel.currentSinger();
        for (int pos = 0; pos < m_rotModel.singerCount(); ++pos)
        {
            const auto &s = m_rotModel.getSingerAtPosition(pos);
            QJsonObject sObj;
            sObj["id"]       = s.id;
            sObj["name"]     = s.name;
            sObj["position"] = s.position;
            sObj["regular"]  = s.regular;
            sObj["current"]  = (s.id == curId);
            singersArray.append(sObj);
        }
        QJsonObject rotObj;
        rotObj["singers"]        = singersArray;
        rotObj["currentSingerId"] = curId;
        m_songbookApi.pushRotationUpdate(rotObj);
    }
...
void MainWindow::tableViewDBContextMenuRequested(const QPoint &pos) {
    ...
    contextMenu.addAction("Preview (video)", [&]() {
        previewKaraokeSong(song->path);
    });
    contextMenu.addAction("Preview (audio, Ctrl+P)", [&]() {
...
void MainWindow::previewSong(const QString &path) {
    if (path.isEmpty() || !QFile::exists(path))
        return;
    // Route to the dedicated preview output device
    QString previewDevice = m_settings.previewAudioDevice();
    if (!previewDevice.isEmpty())
        m_mediaBackendPreview.setAudioOutputDevice(previewDevice);
    m_mediaBackendPreview.stop(true);
    m_mediaBackendPreview.setMedia(path);
    m_mediaBackendPreview.setVolume(30);
    m_mediaBackendPreview.play();
}
...
void MainWindow::karaokeAATimerTimeout() {
    ...
    if (m_kAASkip) {
        ...
        // Notify AutoKJ server and fairness engine of the song now being played
        m_fairnessEngine.onSongPlayed(singer.id, singer.nextSongCoSingers());
        {
            int aaKjSongId = m_karaokeSongsModel.getIdForPath(m_kAANextSongPath);
            m_fairnessEngine.recordSongPlayed(aaKjSongId, singer.id, singer.name);
            for (const QString &coName : singer.nextSongCoSingers()) {
                const auto &coSinger = m_rotModel.getSingerByName(coName);
                m_fairnessEngine.recordSongPlayed(aaKjSongId, coSinger.isValid() ? coSinger.id : -1, coName);
            }
        }
        m_karaokeSongsModel.refreshFairnessState();
        m_songbookApi.notifySongPlayed(m_curSinger, m_curArtist, m_curTitle, singer.nextSongCoSingers(), m_settings.kjName());
        m_songbookApi.pushNowPlaying(m_curSinger, m_curArtist, m_curTitle,
                                     static_cast<int>(m_mediaBackendKar.duration() / 1000), singer.nextSongKeyChg());
    }
}
...
void MainWindow::actionPlaylistImportTriggered() {
    ...
    if (importFile != "") {
        ...
        if (!textFile.open(QFile::ReadOnly)) {
            m_logger->error("{} Failed to open playlist file for import: {}", m_loggingPrefix, importFile.toStdString());
            return;
        }
...
void MainWindow::newVersionAvailable(const QString &version) {
    QMessageBox msgBox;
    msgBox.setTextFormat(Qt::RichText);
    msgBox.setText("New version of Auto-KJ is available: " + version);
...
void MainWindow::autosizeQueueCols() {
    ...
    int iconWidth = fH + fH;
    ui->tableViewQueue->hideColumn(TableModelQueueSongs::COL_SINGER1);
    ...
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER2, QHeaderView::ResizeToContents);
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER3, QHeaderView::ResizeToContents);
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER4, QHeaderView::ResizeToContents);
    ...
    QApplication::processEvents();
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER2, QHeaderView::Interactive);
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER3, QHeaderView::Interactive);
    ui->tableViewQueue->horizontalHeader()->setSectionResizeMode(TableModelQueueSongs::COL_SINGER4, QHeaderView::Interactive);
...
void MainWindow::writeGstPipelineDiagramToDisk() {
    QString outputFolder = QStandardPaths::writableLocation(QStandardPaths::PicturesLocation);
    ...
    std::for_each(selRows.begin(), selRows.end(), [&](auto index) {
        ...
        if (dbSongId == -1) {
            QMessageBox::warning(this,
                                 ...
                                 "Auto-KJ's database has been removed or renamed outside of Auto-KJ\n\n"
                                 "Song: " + path
            );
            return;
        }
...
void MainWindow::onDjSelected(int index) {
    QString dj = m_comboBoxDj->itemText(index);
    if (dj == "None") {
        m_settings.setActiveDj("");
    } else {
        m_settings.setActiveDj(dj);
    }
}

void MainWindow::updateDjList() {
    if (!m_comboBoxDj) return;
    
    QString current = m_settings.activeDj();
    m_comboBoxDj->clear();
    m_comboBoxDj->addItem("None");
    m_comboBoxDj->addItems(m_settings.djList());
    
    int idx = m_comboBoxDj->findText(current);
    if (idx != -1) {
        m_comboBoxDj->setCurrentIndex(idx);
    } else {
        m_comboBoxDj->setCurrentIndex(0);
        m_settings.setActiveDj("");
    }
}
...
void MainWindow::showManageVenuesGigsDialog() {
    ...
    DlgManageVenuesGigs dlg(m_settings, m_songbookApi, this);
    dlg.exec();
}
