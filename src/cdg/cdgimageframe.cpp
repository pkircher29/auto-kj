...
void CdgImageFrame::copyCroppedImagedata(uchar *destbuffer)
{
    ...
    // copy color table
    int paletteSize = static_cast<int>(m_image.colorTable().length() * sizeof(uint));
    memcpy(destpos, m_image.colorTable().data(), paletteSize);

    // GStreamer RGB8P expects a 1024 byte palette (256 colors * 4 bytes)
    // We only have 16 colors (64 bytes), so we zero out the rest to prevent junk data.
    memset(destpos + paletteSize, 0, 1024 - paletteSize);
