...
int main(int argc, char *argv[]) {
    ...
    QDir dir;
    QString logFilePath;
    QString filename = "auto-kj-debug-" + QDateTime::currentDateTime().toString("yyyy-MM-dd") + ".log";
    dir.mkpath(logDir);
    ...
    logger->info("Auto-KJ version {} starting up", OKJ_VERSION_STRING);
...
//    }
    ...
    if (QCoreApplication::applicationDirPath() == "/app/bin") {
        ...
        RunGuard guard("SharedMemorySingleInstanceProtectorAutoKJ");
        if (!guard.tryToRun()) {
            QMessageBox msgBox;
            msgBox.setText("Auto-KJ is already running!");
            msgBox.setInformativeText(
                    "In order to protect the database, you can only run one instance of Auto-KJ at a time.\nExiting now.");
    ...
    if (!settings.lastStartupOk()) {
        QMessageBox msgBox;
        msgBox.setText("Auto-KJ appears to have failed to startup on the last run.");
