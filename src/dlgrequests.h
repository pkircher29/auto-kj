...
private:
    ...
    QString rtClickFile;
    QString curRequestId;
    QString curSelReqSinger;
    QTimer testTimer;
    std::shared_ptr<spdlog::logger> m_reqLogger;
    std::vector<QString> m_prevRequestList;
    AutoKJServerClient &songbookApi;
    Settings m_settings;

public:
    explicit DlgRequests(TableModelRotation &rotationModel, AutoKJServerClient &songbookAPI, QWidget *parent = nullptr);
    int numRequests();
    ~DlgRequests();

signals:
    void addRequestSong(int songId, int singerId, int keyChg, const QStringList &cosingers = {});
