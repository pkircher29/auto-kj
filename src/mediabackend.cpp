...
void MediaBackend::play()
{
    ...
    if (m_cdgMode)
    {
        ...
        if (m_settings.cdgPrescalingEnabled() && m_settings.hardwareAccelEnabled())
        {
            ...
            // For CDG mode, we always need videoconvert because RGB8P isn't natively supported by most sinks
            gst_element_unlink_many(m_queueMainVideo, m_prescalerVideoConvert, m_prescaler, m_prescalerCapsFilter, m_videoTee, nullptr);
            gst_element_link_many(m_queueMainVideo, m_prescalerVideoConvert, m_videoTee, nullptr);
        }
...
void MediaBackend::buildAudioSinkBin()
{
    ...
    gst_bin_add_many(GST_BIN(m_audioBin), queueMainAudio, audioResample, m_audioPanorama, level, m_scaleTempo, aConvInput, rgVolume, m_volumeElement, m_equalizer, aConvPostPanorama, m_fltrPostPanorama, m_faderVolumeElement, nullptr);
    gst_element_link_many(queueMainAudio, aConvInput, audioResample, rgVolume, m_scaleTempo, level, m_equalizer, m_audioPanorama, aConvPostPanorama, audioBinLastElement = m_fltrPostPanorama, nullptr);
...
#ifdef Q_OS_WIN
        m_audioSink = gst_element_factory_make("directsoundsink", "audioSink");
        if (!m_audioSink)
