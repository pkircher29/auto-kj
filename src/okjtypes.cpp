...
namespace okj {
    ...
    QStringList RotationSinger::nextSongCoSingers() const {
        QSqlQuery query;
        query.prepare("SELECT cosinger2, cosinger3, cosinger4 FROM queuesongs WHERE singer = :singerid AND played = 0 ORDER BY position LIMIT 1");
        query.bindValue(":singerid", id);
        query.exec();
        QStringList ret;
        if (query.first()) {
            if (!query.value(0).toString().isEmpty()) ret << query.value(0).toString();
            if (!query.value(1).toString().isEmpty()) ret << query.value(1).toString();
            if (!query.value(2).toString().isEmpty()) ret << query.value(2).toString();
        }
        return ret;
    }
