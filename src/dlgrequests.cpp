...
DlgRequests::DlgRequests(TableModelRotation &rotationModel, AutoKJServerClient &songbookAPI, QWidget *parent) :
        QDialog(parent),
    ...
    dir.mkpath(logDir);
    QString logFilePath;
    QString filename = "Auto-KJ-requests-" + QDateTime::currentDateTime().toString("yyyy-MM-dd") + ".log";
    ...
    connect(&songbookApi, &AutoKJServerClient::synchronized, this, &DlgRequests::updateReceived);
    connect(&songbookApi, &AutoKJServerClient::sslError, this, &DlgRequests::sslError);
    connect(&songbookApi, &AutoKJServerClient::delayError, this, &DlgRequests::delayError);
    ...
    posOptions << tr("Fairness Engine (full rotation)");
    ...
    connect(&songbookApi, &AutoKJServerClient::venuesChanged, this, &DlgRequests::venuesChanged);
    ...
    // Venue selection and accepting controls moved to the top toolbar.
    ui->label_3->hide();
    ui->comboBoxVenue->hide();
    ui->checkBoxAccepting->hide();
    ...
    connect(&songbookApi, &AutoKJServerClient::requestsChanged, this, &DlgRequests::requestsChanged);
...
void DlgRequests::requestSelectionChanged(const QItemSelection &current, const QItemSelection &previous) {
    ...
    if ((index.isValid()) && (ui->tableViewRequests->selectionModel()->selectedIndexes().size() > 0)) {
        ...
        const Request &req = requestsModel->requests().at(index.row());
        ui->lineEditCoSinger2->setText(req.cosinger2());
        ui->lineEditCoSinger3->setText(req.cosinger3());
        ui->lineEditCoSinger4->setText(req.cosinger4());
...
void DlgRequests::on_tableViewRequests_clicked(const QModelIndex &index) {
    if (index.column() == 5) {
        const QString requestId = index.data(Qt::UserRole).toString();
        songbookApi.removeRequest(requestId);
        m_reqLogger->info("RequestID: {} | Manually removed by host", requestId);
        m_reqLogger->flush();
        ui->tableViewRequests->selectionModel()->clearSelection();
        ui->lineEditSearch->clear();
        ui->lineEditSingerName->clear();
        ...
        curRequestId = index.data(Qt::UserRole).toString();
    }
}
...
void DlgRequests::on_pushButtonAddSong_clicked() {
    ...
    if (!ui->lineEditCoSinger2->text().trimmed().isEmpty()) cosingers << ui->lineEditCoSinger2->text().trimmed();
    if (!ui->lineEditCoSinger3->text().trimmed().isEmpty()) cosingers << ui->lineEditCoSinger3->text().trimmed();
    if (!ui->lineEditCoSinger4->text().trimmed().isEmpty()) cosingers << ui->lineEditCoSinger4->text().trimmed();

    if (ui->radioButtonNewSinger->isChecked()) {
        QString newName = ui->lineEditSingerName->text();
        if (newName == "" || rotModel.singerExists(newName))
            return;
            
        SingerIdentityManager identityManager(rotModel);
        auto match = identityManager.checkForDuplicate(newName);
        ...
        if (match.isDuplicate) {
            ...
            if (msgBox.clickedButton() == mergeBtn || msgBox.clickedButton() == aliasBtn) {
                if (msgBox.clickedButton() == aliasBtn) {
                ...
                emit addRequestSong(song->id, match.singerId, keyChg, cosingers);
                songbookApi.removeRequest(curRequestId);
                return;
            } else if (msgBox.clickedButton() != addAnywayBtn) {
                return; // Cancelled
            }
        }

        int newSingerId = rotModel.singerAdd(newName, ui->comboBoxAddPosition->currentIndex());
        emit addRequestSong(song->id, newSingerId, keyChg, cosingers);
        m_reqLogger->info(
                "RequestID: {} | Added to new singer | Name: {} | Position: {} | Wait: {} | Song: {} - {} - {} | Key: {} | Co-singers: {}",
                curRequestId,
                ...
                song->title,
                keyChg,
                cosingers.join(", ").toStdString()
        );
        m_reqLogger->flush();
    } else if (ui->radioButtonExistingSinger->isChecked()) {
        emit addRequestSong(song->id, rotModel.getSingerByName(ui->comboBoxSingers->currentText()).id, keyChg, cosingers);
        m_reqLogger->info("RequestID: {} | Added to existing singer | Name: {} | Song: {} - {} - {} | Key: {} | Co-singers: {}",
                          curRequestId,
    ...
    if (m_settings.requestRemoveOnRotAdd()) {
        ...
        ui->lineEditSingerName->clear();
        ui->comboBoxSingers->clear();
        ui->lineEditCoSinger2->clear();
        ui->lineEditCoSinger3->clear();
        ui->lineEditCoSinger4->clear();
        ui->radioButtonExistingSinger->setChecked(true);
    }
}
...
//        progressDialog->setCancelButton(0);
        ...
        progressDialog->show();
        QApplication::processEvents();
        connect(&songbookApi, &AutoKJServerClient::remoteSongDbUpdateNumDocs, progressDialog, &QProgressDialog::setMaximum);
        connect(&songbookApi, &AutoKJServerClient::remoteSongDbUpdateProgress, progressDialog, &QProgressDialog::setValue);
        connect(progressDialog, &QProgressDialog::canceled, &songbookApi, &AutoKJServerClient::dbUpdateCanceled);
        // When sync completes (async), close progress and notify user.
        // Use progressDialog as context so these connections auto-disconnect when it is destroyed.
        connect(&songbookApi, &AutoKJServerClient::remoteSongDbUpdateDone, progressDialog, [this, progressDialog]() {
            qInfo() << "Closing progress dialog for remote db update";
            progressDialog->close();
            progressDialog->deleteLater();
            QMessageBox::information(this, tr("Update complete"), tr("Remote database update completed!"));
            ui->pushButtonUpdateDb->setEnabled(true);
        });
        connect(&songbookApi, &AutoKJServerClient::remoteSongDbUpdateFailed, progressDialog, [this, progressDialog](const QString &error) {
            qInfo() << "Remote db update failed:" << error;
            progressDialog->close();
            progressDialog->deleteLater();
            QMessageBox::warning(this, tr("Update failed"), tr("Remote database update failed:\n\n") + error);
            ui->pushButtonUpdateDb->setEnabled(true);
        });
        connect(progressDialog, &QProgressDialog::canceled, progressDialog, [this, progressDialog]() {
            progressDialog->deleteLater();
            ui->pushButtonUpdateDb->setEnabled(true);
        });
        songbookApi.updateSongDb();
        return;  // completion handled via signals above
    }
    ui->pushButtonUpdateDb->setEnabled(true);
}
...
void DlgRequests::requestsChanged(OkjsRequests requests) {
    std::vector<QString> curRequestsList;
