...
{
    return settings->value("storeDownloadDir", QStandardPaths::writableLocation(QStandardPaths::MusicLocation) + QDir::separator() + "Auto-KJ_Downloads" + QDir::separator()).toString();
}

QString Settings::logDir()
{
    return settings->value("logDir", QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + QDir::separator() + "Auto-KJ_Logs" + QDir::separator()).toString();
}
...
// REMOVED: CC storage functions (getCCN/getCCM/getCCY/getCCV/setCC) — use Stripe
// REMOVED: KaraokeDotNet functions (setKaroakeDotNetUser/setKaraokeDotNetPass/etc) — dead code, broken crypto
    ...
    QObject(parent)
{
    ...
    settings = new QSettings(khDir.absolutePath() + QDir::separator() + "auto-kj.ini", QSettings::IniFormat);
#endif
}
...
{
    return settings->value("requestServerEnabled", true).toBool();
}
...
{
    return settings->value("requestServerVenue", 1).toInt();
}
...
{
    SecureCredentialStore store;
    QString secure = store.load("AutoKJ/requestServerApiKey");
    if (!secure.isEmpty())
...
void Settings::setRequestServerApiKey(const QString &apiKey)
{
    ...
    if (requestServerApiKey() == trimmed)
        return;
    SecureCredentialStore store;
    store.store("AutoKJ/requestServerApiKey", trimmed);
...
QString Settings::requestServerVenueSlug() const
{
    return settings->value("requestServer/venueSlug", QString()).toString();
}

void Settings::setRequestServerVenueSlug(const QString &slug)
{
    settings->setValue("requestServer/venueSlug", slug);
}

QString Settings::venueShowtimesJson(int venueId) const
{
    return settings->value(QString("requestServer/showtimes/%1").arg(venueId), "[]").toString();
}

void Settings::setVenueShowtimesJson(int venueId, const QString &json)
{
    settings->setValue(QString("requestServer/showtimes/%1").arg(venueId), json);
}

QString Settings::kjName()
{
    QString active = activeDj();
    if (!active.isEmpty())
        return active;
    return settings->value("kjName", QString()).toString();
}

void Settings::setKjName(const QString &name)
{
    settings->setValue("kjName", name);
}

QStringList Settings::djList() const
{
    return settings->value("djList").toStringList();
}

void Settings::setDjList(const QStringList &djs)
{
    settings->setValue("djList", djs);
    emit djListChanged();
}
...
void Settings::setActiveDj(const QString &dj)
{
    settings->setValue("activeDj", dj);
    emit activeDjChanged(dj);
    emit gigSettingsChanged(); // Trigger sync to server with new host name/rules
}

QString Settings::djKey(const QString &baseKey) const
{
    QString dj = activeDj();
    if (dj.isEmpty())
        return baseKey;
    return QString("DJ/%1/%2").arg(dj, baseKey);
}

// DJ-specific settings for current singer limits and fairness rules are defined later in the file

QString Settings::previewAudioDevice()
{
    return settings->value("previewAudioDevice", QString()).toString();
}

void Settings::setPreviewAudioDevice(const QString &device)
{
    settings->setValue("previewAudioDevice", device);
}
...
QString Settings::venueAddress() const {
    return settings->value("venueAddress", "").toString();
}

void Settings::setVenueAddress(const QString &address) {
    settings->setValue("venueAddress", address);
    emit venueConfigChanged();
}

double Settings::venueLat() const {
    return settings->value("venueLat", 0.0).toDouble();
}

void Settings::setVenueLat(double lat) {
    settings->setValue("venueLat", lat);
    emit venueConfigChanged();
}

double Settings::venueLon() const {
    return settings->value("venueLon", 0.0).toDouble();
}

void Settings::setVenueLon(double lon) {
    settings->setValue("venueLon", lon);
    emit venueConfigChanged();
}

double Settings::venueGeofenceRadius() const {
    return settings->value("venueGeofenceRadius", 1.0).toDouble();
}

void Settings::setVenueGeofenceRadius(double miles) {
    settings->setValue("venueGeofenceRadius", miles);
    emit venueConfigChanged();
}

bool Settings::venueGeofenceEnabled() const {
    return settings->value("venueGeofenceEnabled", true).toBool();
}

void Settings::setVenueGeofenceEnabled(bool enabled) {
    settings->setValue("venueGeofenceEnabled", enabled);
    emit venueConfigChanged();
}
...
void Settings::setBlockRepeatSongs(bool enabled) {
    settings->setValue(djKey("blockRepeatSongs"), enabled);
    emit gigSettingsChanged();
}

bool Settings::limitOneSongPerSinger() const {
    return settings->value(djKey("limitOneSongPerSinger"), true).toBool();
}

void Settings::setLimitOneSongPerSinger(bool enabled) {
    settings->setValue(djKey("limitOneSongPerSinger"), enabled);
    emit gigSettingsChanged();
}

int Settings::kjBufferSeconds() const {
    return settings->value(djKey("kjBufferSeconds"), 45).toInt();
}

void Settings::setKjBufferSeconds(int seconds) {
    settings->setValue(djKey("kjBufferSeconds"), seconds);
    emit gigSettingsChanged();
}

int Settings::noShowTimeoutSeconds() const {
    return settings->value(djKey("noShowTimeoutSeconds"), 30).toInt();
}

void Settings::setNoShowTimeoutSeconds(int seconds) {
    settings->setValue(djKey("noShowTimeoutSeconds"), seconds);
    emit gigSettingsChanged();
}

bool Settings::lineJumpEnabled() const {
    return settings->value(djKey("lineJumpEnabled"), false).toBool();
}

void Settings::setLineJumpEnabled(bool enabled) {
    settings->setValue(djKey("lineJumpEnabled"), enabled);
    emit gigSettingsChanged();
}

bool Settings::fairnessEnabled() const {
    const bool toggledOn = settings->value(djKey("fairnessEnabled"), true).toBool();
    return toggledOn && antiChaosPremiumEnabled();
}

void Settings::setFairnessEnabled(bool enabled) {
    settings->setValue(djKey("fairnessEnabled"), enabled);
    emit gigSettingsChanged();
}
...
bool Settings::premiumAntiChaosAuthorized() const {
    return settings->value("runtime/premiumAntiChaosAuthorized", false).toBool();
}

void Settings::setPremiumAntiChaosAuthorized(bool authorized) {
    settings->setValue("runtime/premiumAntiChaosAuthorized", authorized);
    emit gigSettingsChanged();
}

bool Settings::fairnessEnforceRound() const {
    return settings->value(djKey("fairnessEnforceRound"), true).toBool();
}

void Settings::setFairnessEnforceRound(bool enabled) {
    settings->setValue(djKey("fairnessEnforceRound"), enabled);
    emit gigSettingsChanged();
}

bool Settings::fairnessEnforceSongNight() const {
    return settings->value(djKey("fairnessEnforceSongNight"), true).toBool();
}

void Settings::setFairnessEnforceSongNight(bool enabled) {
    settings->setValue(djKey("fairnessEnforceSongNight"), enabled);
    emit gigSettingsChanged();
}

bool Settings::fairnessDetectNameChange() const {
    return settings->value(djKey("fairnessDetectNameChange"), true).toBool();
}

void Settings::setFairnessDetectNameChange(bool enabled) {
    settings->setValue(djKey("fairnessDetectNameChange"), enabled);
    emit gigSettingsChanged();
}

void Settings::setMaxSingersPerRequest(int max) {
    settings->setValue(djKey("requestServerMaxSingersPerRequest"), max);
    emit gigSettingsChanged();
}

int Settings::maxSingersPerRequest() const {
    return settings->value(djKey("requestServerMaxSingersPerRequest"), 4).toInt();
}

int Settings::maxQueuePerSinger() const {
    return settings->value(djKey("maxQueuePerSinger"), 2).toInt();
}

void Settings::setMaxQueuePerSinger(int limit) {
    settings->setValue(djKey("maxQueuePerSinger"), qMax(1, limit));
    emit gigSettingsChanged();
}
