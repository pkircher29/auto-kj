...
DlgSettings::DlgSettings(MediaBackend &AudioBackend, MediaBackend &BmAudioBackend, AutoKJServerClient &songbookAPI,
                         QWidget *parent) :
        QDialog(parent),
    ...
    auto qrWidget = new QRCodeWidget(this);
    qrWidget->setObjectName("qrWidget");
    qrWidget->setUrl(ui->lineEditUrl->text());
    qrWidget->setMinimumSize(250, 250);
    ui->groupBoxRequestServer->layout()->addWidget(qrWidget);

    auto hlayMax = new QHBoxLayout;
    hlayMax->addWidget(new QLabel("Max Singers per Request:"));
    auto spinMaxSingers = new QSpinBox(this);
    spinMaxSingers->setObjectName("spinMaxSingers");
    spinMaxSingers->setRange(1, 4);
    ...
    connect(spinMaxSingers, qOverload<int>(&QSpinBox::valueChanged),
            this, &DlgSettings::maxSingersPerRequestChanged);

    auto cbxLimitOneSong = new QCheckBox("Limit One Song per Singer", this);
    cbxLimitOneSong->setChecked(m_settings.limitOneSongPerSinger());
    ui->groupBoxRequestServer->layout()->addWidget(cbxLimitOneSong);
    connect(cbxLimitOneSong, &QCheckBox::toggled, &m_settings, &Settings::setLimitOneSongPerSinger);

    auto cbxBlockRepeatSongs = new QCheckBox("Block Repeat Songs", this);
    cbxBlockRepeatSongs->setChecked(m_settings.blockRepeatSongs());
    ui->groupBoxRequestServer->layout()->addWidget(cbxBlockRepeatSongs);
    connect(cbxBlockRepeatSongs, &QCheckBox::toggled, &m_settings, &Settings::setBlockRepeatSongs);

    auto cbxLineJumpEnabled = new QCheckBox("Line Jump Enabled (Fairness Priority)", this);
    cbxLineJumpEnabled->setChecked(m_settings.lineJumpEnabled());
    ui->groupBoxRequestServer->layout()->addWidget(cbxLineJumpEnabled);
    connect(cbxLineJumpEnabled, &QCheckBox::toggled, &m_settings, &Settings::setLineJumpEnabled);

    auto hlayNoShow = new QHBoxLayout;
    hlayNoShow->addWidget(new QLabel("No-show Timeout (seconds):"));
    auto spinNoShow = new QSpinBox(this);
    spinNoShow->setRange(10, 300);
    ...
    hlayKjBuffer->addWidget(new QLabel("KJ Buffer (seconds):"));
    auto spinKjBuffer = new QSpinBox(this);
    spinKjBuffer->setRange(10, 300);
    ...
    cbxGeofenceEnabled->setChecked(m_settings.venueGeofenceEnabled());
    ui->groupBoxRequestServer->layout()->addWidget(cbxGeofenceEnabled);
    connect(cbxGeofenceEnabled, &QCheckBox::toggled, &m_settings, &Settings::setVenueGeofenceEnabled);

    auto hlayGeoRadius = new QHBoxLayout;
    hlayGeoRadius->addWidget(new QLabel("Geofence Radius (miles):"));
    auto spinGeoRadius = new QDoubleSpinBox(this);
    spinGeoRadius->setRange(0.1, 50.0);
    spinGeoRadius->setSingleStep(0.5);
    ...
    ui->checkBoxFairnessEnabled->setChecked(m_settings.fairnessEnabled());
    ui->checkBoxFairnessEnforceRound->setChecked(m_settings.fairnessEnforceRound());
    ui->checkBoxFairnessEnforceSongNight->setChecked(m_settings.fairnessEnforceSongNight());
    ui->checkBoxFairnessDetectNameChange->setChecked(m_settings.fairnessDetectNameChange());
    adjustSize();
    tickerFontChanged();
    ...
    connect(ui->checkBoxFairnessEnabled, &QCheckBox::toggled, &m_settings, &Settings::setFairnessEnabled);
    connect(ui->checkBoxFairnessEnforceRound, &QCheckBox::toggled, &m_settings, &Settings::setFairnessEnforceRound);
    connect(ui->checkBoxFairnessEnforceSongNight, &QCheckBox::toggled, &m_settings, &Settings::setFairnessEnforceSongNight);
    connect(ui->checkBoxFairnessDetectNameChange, &QCheckBox::toggled, &m_settings, &Settings::setFairnessDetectNameChange);
...
void DlgSettings::setupHotkeysForm() {
    ...
    for (auto &shortcut : shortcuts) {
        ...
        clearButton->setIcon(QIcon(":/theme/Icons/autokjbreeze-dark/actions/22/edit-clear.svg"));
...
void DlgSettings::on_lineEditUrl_editingFinished() {
    ...
    auto qr = this->findChild<QRCodeWidget*>("qrWidget");
    if(qr) qr->setUrl(ui->lineEditUrl->text());
}
